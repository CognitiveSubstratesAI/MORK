#!/usr/bin/env bash
# suite_result.sh — the ONLY sanctioned answer to "did the MORK suite pass?"
#
...
#   PARTIAL -> 5     the last run targeted ONE FILE (a probe), or has no TARGET field at all
...
# A verdict from a SINGLE-FILE run is not a suite verdict, and one with no TARGET field cannot be
# attributed at all. Probes go through the same runner (measured 2026-08-25: probe runs left a
# PASS describing two lines of scratch code, against a 0-byte suite log).
if [ -z "$TARGET" ]; then
