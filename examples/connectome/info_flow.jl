...
const SYN_PREFIX = vcat(
    UInt8[item_byte(ExprArity(UInt8(4))), item_byte(ExprSymbol(UInt8(3)))],
    codeunits("syn"))
...
@inline function _parse_pre_post_cnt(rel::Vector{UInt8})
    ...
    pos = 2 + plen
    pos > length(rel) && return nothing
    pc = _parse_post_cnt(rel[pos:end])
...
function oracle_ranks(
    out, tin, seeds::Vector{String}; threshold::Float64=0.3, max_rounds::Int=30
)
    reached = Set{String}(seeds)
    ranks = Dict{String, Int}(sd => 0 for sd in seeds)
    for k in 1:max_rounds
...
function validate(m, modality::AbstractString)
    ...
    mism =
        sum(get(fr, k, -999) != v for (k, v) in orc; init=0) +
...
function all_modalities(m=ConnectomeManifest.manifest();
    ...
    for k in 0:maxr
        print(rpad(k, 6))
        for mod in modalities
            print(rpad(count(==(k), values(results[mod])), 15))
        end
        println()
    end
