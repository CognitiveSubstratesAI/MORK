...
count_conj(s) = (n = Ref(0); MORK.space_query_multi(s, MORK.sexpr_to_expr(CONJ), (a...) -> (n[]+=1; true)); n[])

# (A) state at counter=Z reached by DECREMENT (run Control_08 four steps)
sa = new_space(); space_add_all_sexpr!(sa, PROG); space_metta_calculus!(sa, 4)

# (B) the SAME visible atoms, FRESH-built
fresh = "(counter Z)\n(exec LOOP (, (counter (S \$N)) (exec LOOP \$p \$t)) " *
        "(O (+ (exec LOOP \$p \$t)) (+ (counter \$N)) (- (counter (S \$N)))))\n"
sb = new_space(); space_add_all_sexpr!(sb, fresh)
...
i = Ref(0)
MORK.space_query_multi(sa, MORK.sexpr_to_expr(CONJ), (binds, expr) -> begin
    i[] += 1
    println("  match $(i[]): ", MORK.expr_serialize(expr isa MORK.Expr ? expr.buf : expr))
    true
end)
