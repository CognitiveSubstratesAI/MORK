...
println(
    "  get_val_at    = ", MORK.get_val_at(s.btm, buf) === nothing ? "ABSENT" : "PRESENT"
)
...
println(
    "  conj matches  = ",
    n[],
    "   (0 = clean; >0 = remove leaves a query-visible dangling path)"
)
...
while MORK.zipper_to_next_val!(rz)
    push!(
        paths,
        try
            MORK.expr_serialize(copy(rz.prefix_buf))
        catch
            "<?>"
        end
    )
