...
function run_and_save()
    ...
    open(outfile, "w") do f
        ...
        println(f, "")
        println(f, "Fixtures: `tools/diff_upstream.jl` UPSTREAM_INPUTS (upstream `kernel/src/main.rs`)",
            isfile(_EXTRA) ? " + `experiments/upstream_diff/data_in_mork.jl`" : "")
        println(f, "")
        println(f, "**$pass PASS · $fail FAIL · $crash CRASH · $miss MISS** of $(length(UPSTREAM_INPUTS)) fixtures")
        println(f, "")
    ...
    println("UPSTREAM_DIFF_RESULT: $pass PASS  $fail FAIL  $crash CRASH  $miss MISS  →  $outfile")
