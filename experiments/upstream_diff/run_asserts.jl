...
function run_and_save()
    ...
    for fx in MAIN_RS_FIXTURES
        ...
        try
            ...
            if isempty(missing)
                if fx.wip
                    (wippass += 1; push!(rows, (fx.name, "PASS(wip)", String[])))
                else
                    (pass += 1; push!(rows, (fx.name, "PASS", String[])))
                end
            elseif fx.wip
                wipfail += 1
                push!(rows, (fx.name, "FAIL(wip)", missing))
            else
                fail += 1
                push!(rows, (fx.name, "FAIL", missing))
            end
        catch e
            crash += 1
            push!(rows, (fx.name, "CRASH", [first(split(sprint(showerror, e), '\n'))]))
        end
    end
    ...
    open(outfile, "w") do f
        ...
        println(f,
            "Checks upstream `main.rs` `assert!(res.contains(...))` (the real spec). `(wip)` = upstream",
            " comments the test out in its own `main()` (WIP/known-faulty), not a port regression.\n"
        )
        println(f,
            "**$pass PASS · $fail FAIL · $crash CRASH** (real)  +  $wippass PASS(wip) · $wipfail FAIL(wip)",
            "  of $(length(MAIN_RS_FIXTURES))\n")
        println(f, "| case | status | missing assert |")
        println(f, "|---|---|---|")
        for (n, s, miss) in rows
            println(
                f, "| `$n` | $s | $(isempty(miss) ? "" : "`" * join(miss, "`, `") * "`") |"
            )
        end
    end
    println(
        "ASSERT_RESULT: $pass PASS  $fail FAIL  $crash CRASH  (+ $wippass/$wipfail wip pass/fail)  → $outfile"
    )
    for (n, s, _) in rows
        (s == "PASS" || s == "PASS(wip)") || println("  $s  $n")
    end
end
