...
Instruction(op::Op) =
    Instruction(op, PathStoreRef(), (UInt16(0), UInt16(0), UInt16(0), UInt16(0)))
...
function interp_add_routine!(
    interp::Interpreter, name::AbstractVector{UInt8}, routine::RoutineImpl
)
...
function run_routine(interp::Interpreter, routine_with_arguments::AbstractVector{UInt8},
    ...
    call_stack = CallStack(
        collect(routine_with_arguments), Int[0, length(routine_with_arguments)]
    )
        ...
        while true   # 'subroutine loop
            ...
            elseif op == OP_ITER_SUBROUTINE
                ...
                if first_byte === nothing
                    ...
                    push!(
                        sub_stack,
                        Subroutine(zspace, rz, it, subr_start,
